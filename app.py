...
def registered():
    ...
    row = db.execute("SELECT * FROM student WHERE email=?",
                     user_email)  # Check if account already exists
    if len(row) == 1:
        # Throw error if exists
        return render_template("error.html", message="Account already exists")
    else:
        db.execute("INSERT INTO student (email, password) VALUES (?,?)",
                   user_email, user_pass)  # Else add user info to DB
...
def mainpage():
    ...
    row = db.execute("SELECT * FROM student WHERE email=? AND password=?",
                     user_email, user_pass)  # Confirm login authentication
    if len(row) == 0:
